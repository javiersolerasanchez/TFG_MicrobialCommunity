...
def runn(md = ""):
    ...
    mod_dir = "/home/javi/Documentos/Combination_pert/" #Directorio donde está el modelo
    ...
    for pert in range(3): # BGJ: 2019.11.08 #SI QUEREMOS CAMBIAR EL NÚMERO DE PERTURBACIONES POR SIMULACIÓN, ES AQUÍ.
        ...
        if chosen > 0.66:
            ...
            gen_media[-1]["PERTURBATION"] = st_ids[0] + "_" + 'PHOSPHATE' #Nos referimos al último diccionario, al key "PERTURBATION" y lo cambiamos por ese valor.
        elif chosen > 0.33: # 2019.11.12: BGJ: to increase biomass 2nd strain: st_ids[1]
            gen_media[-1]["MEDIA"][st_ids[1]] = fix_biomass*100
            gen_media[-1]["MEDIA"]['cpd00009_e0'] = 1
            gen_media[-1]["PERTURBATION"] = st_ids[1] + "_" + 'PHOSPHATE'
        else:
            gen_media[-1]["PERTURBATION"] = st_ids[0] + "_" + st_ids[1] + "_" + 'PHOSPHATE'
            gen_media[-1]["MEDIA"]['cpd00009_e0'] = 1
